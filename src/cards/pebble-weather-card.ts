...
/** Idle time after the user scrolls the forecast before it resets to the start. */
const FORECAST_IDLE_RESET_MS = 60_000;
...
class PebbleWeatherCard extends LitElement {
  ...
  private _forecastScrollResetTimer?: number;
  /** True while we are programmatically scrolling the list back to the start. */
  private _programmaticForecastScroll = false;
  ...
  disconnectedCallback() {
    ...
    this._cancelForecastScrollReset();
  }
  ...
  private _onForecastScroll() {
    ...
    // Ignore scroll events generated by our own reset so we don't re-arm the
    // idle timer in an endless loop. The flag is cleared once we reach the
    // start of the list.
    if (this._programmaticForecastScroll) {
      const listEl = this._forecastListRef.value;
      if (!listEl || listEl.scrollLeft <= 1) {
        this._programmaticForecastScroll = false;
      }
      return;
    }

    this._scheduleForecastScrollReset();
  }

  /** (Re)start the idle countdown that returns the forecast to its start. */
  private _scheduleForecastScrollReset() {
    this._cancelForecastScrollReset();
    this._forecastScrollResetTimer = window.setTimeout(
      () => this._resetForecastScroll(),
      FORECAST_IDLE_RESET_MS,
    );
  }

  private _cancelForecastScrollReset() {
    if (this._forecastScrollResetTimer !== undefined) {
      clearTimeout(this._forecastScrollResetTimer);
      this._forecastScrollResetTimer = undefined;
    }
  }

  private _resetForecastScroll() {
    this._forecastScrollResetTimer = undefined;
    ...
    this._programmaticForecastScroll = true;
    listEl.scrollTo({ left: 0, behavior: "smooth" });
  }
